...
function login_nav(p, arg) {
  $("#login-form").submit(function (e) {
    ...
    socket.sendMessage("login", data, function (e, d) {
      ...
      if (recordar)
        localStorage.setItem("srq_lot_srv_login", JSON.stringify(data));
    });
  });
}
...
function sorteoRegistrar_nav(p, arg) {
  ...
  $("#sorteo-registrar").submit(function (e) {
    ...
    function sorteo_registrar_handler(e, d) {
      notificacion(
        "SORTEO(S) REGISTRADO(S)",
        "<p>Fecha: " +
          data.fecha +
          "</p><p>" +
          d.length +
          " sorteos registrados</p>"
      );
...
function bancasUsuarios_nav() {
  if ($usuarios) updateView();
    ...
    socket.sendMessage("usuarios", null, function (e, d) {
      $usuarios = d || [];
      updateView();
    });
  ...
  $("#usuario-nuevo").submit(function (e) {
    ...
    socket.sendMessage("usuario-nuevo", data, function (e, d) {
      ...
      if (d > 0) {
        ...
        notificacion(
          "USUARIO DUPLICADO",
          "Imposible registrar usuario, ya existe",
          "growl-danger"
        );
      }
    });
  ...
  if (args && args.length == 1) {
    ...
    function bancaResult() {
      ...
      remTaqI.click(function () {
        ...
        var r = confirm(
          "Confirma desea remover todas las taquillas inactivas?"
        );
...
function reporteComercial_nav(p, args) {
  ...
  function prepareDownload() {
    ...
    var name = (name =
      "SRQ - REPORTE " + fdata.inicio + "-" + fdata.fin + "." + formato.val());
    if (formato.val() == "json") {
...
function sql_nav(p, args) {
  ...
  updateUlt();
  const cache = localStorage.getItem("srq.sqlcache.last");
  if (cache) window.editor.setValue(cache);
  ...
  form.submit(function (e) {
    ...
    var data = formControls(this);
    formLock(this);
    localStorage.setItem("srq.sqlcache.last", data.sql);
    sqlq = data.sql.split(";\n");
