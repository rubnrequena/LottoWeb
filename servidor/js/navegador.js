...
function sorteoMonitor_nav(e, arg) {
  ...
  function actualizarVista() {
    $('.mnt-historial').click(function (e) {
      ...
      socket.sendMessage("sorteo-num-hist", {
        n: num
      }, function (e, result) {
    ...
    $('.mnt-vnt-tickets').click(function (e) {
      e.preventDefault(e);
      var num = $(this).attr("n");
      socket.sendMessage("sorteo-monitor-vnt", {
        numero: num,
      ...
      function premiar(data, elm) {
        socket.sendMessage("sorteo-premiar", data, function (e, d) {
          ...
          if (d.code == 1) {
            ...
            if (r) {
              socket.sendMessage("sorteo-reiniciar", {
...
function sorteoNuevo_nav(p, arg) {
  ...
  function updSorteos() {
    ...
    $('.sorteo-rem').click(function (e) {
      ...
      socket.sendMessage("presorteo-remover", {
        sorteoID: id
      }, function (e, d) {
  ...
  $('#remsorteo').click(function (e) {
    ...
    if (c) {
      socket.sendMessage('sorteo-remover', {
        sID: id
      }, function (e, d) {
...
function sorteoFrutas_nav(p, arg) {
  ...
  $('#fzodiaco').submit(function (e) {
    ...
    socket.sendMessage("elemento-nuevo-zodiaco", {
      sorteo: id,
      numeros: data,
...
function bancasUsuario_nav(p, args) {
  ...
  if (args && args.length == 1) {
    socket.sendMessage("usuarios", {
      id: args[0]
    }, function (e, d) {
      usuario = d[0];
      formSet($('#usuario-editar'), usuario);

      socket.sendMessage('bancas', {
...
function bancasBanca_nav(p, args) {
  if (args && args.length == 1) {
    ...
    nvoGrupo.submit(function (e) {
      e.preventDefault()
      notificacion('En construccion')
    })
...
function reporteComercial_nav(p, args) {
  ...
  $('#reporte').submit(function (e) {
    ...
    socket.sendMessage("reporte-comercial", {
      s: fdata,
      g: grp.val()
    }, function (e, d) {
      formLock(f, false);
...
function reporteCobros_nav(p, args) {
  ...
  reporte.submit(function (e) {
    ...
    socket.sendMessage("reporte-cobros", {
      ...
      function activarBotonesMenu(id = '') {
        ...
        $(`.cbr${id}-disable`).off("click");
        $(`.cbr${id}-disable`).click(function (e) {
          e.preventDefault(e);
  ...
  $('#cbr-procesar').click(function () {
    ...
    function lzLoad(item) {
      ...
      var descripcion = "COBRO SRQ " + f1.val() + "|" + f2.val() + ", " + $(item).attr('desc')
      if (m < min) {
        m = min;
        descripcion = "COBRO SRQ " + f1.val() + "|" + f2.val() + ", " + $(item).attr('desc') + " *RENTA BASICA*"
      }
      //descuento
      if (descuento > 0) {
        descripcion = "COBRO SRQ " + f1.val() + "|" + f2.val() + ", " + $(item).attr('desc') + " *TOTAL: " + m.toFixed(2) + " -" + descuento + "% DESCUENTO*"
        m = m - (m * descuento / 100)
      }
...
function reporteBalance_nav(p, args) {
  ...
  if (args && args.length > 0) {
    ...
    if (user) balance();
    else socket.sendMessage('usuarios', {
      id: a
    }, function (e, d) {
      user = d[0];
      balance();
    ...
    function balance() {
      ...
      socket.sendMessage('balance-general', {
        usID: user.usID
      }, function (e, d) {
        usData = d || [];
        if (usData.length > 0) {
        ...
        function updateView() {
          if (balPend) {
            ...
            $('.bl-rmv-pend').click(function (e) {
              ...
              socket.sendMessage('balance-remover-pend', {
                bID: id
              }, function (e, d) {
  ...
  $('#mt-ms').click(function () {
    if (mstimer == 0) {
      ...
      mstimer = setInterval(function () {
        socket.sendMessage("sys-monitor", {
          m: "ms"
        }, ms_result);
      }, 1000);
    } else {
