  ...
  for (i = 0; i < formFields.length; i++) {
    ...
    if (input.name) {
      if (input.type == "text") needGroup(input.value, input.name);
        ...
        needGroup(parseFormItem(input.value), input.name);
      else if (input.type == "bool")
        needGroup(input.value == "true", input.name);
      else if (input.type == "checkbox") {
        if (input.checked)
          needGroup(parseFormItem(input.value), input.name, true);
...
function formLock(form, lock, delay) {
  var submit = $(form).find(":submit");
  submit.prop("disabled", lock == undefined ? true : lock);
  if (delay) {
    setTimeout(() => submit.prop("disabled", false), delay);
  }
...
var _helpers = {
  ...
  padding: padding,
  toString: function (v) {
    return v ? escapeHtml(JSON.stringify(v)) : "";
  },
  rnd: function (pad) {
    pad = pad || 1000000;
  ...
  markdown: function markdownHelper(contenido) {
    ...
    while ((m = r.exec(contenido)))
      contenido = contenido.replace(
        m[0],
        `<strong class="text-muted">${m[1]}</strong>`
      );
    return contenido;
  },
};
...
function toParams(o) {
  var params = [];
  for (var prop in o) {
    params.push(`${prop}=${encodeURI(o[prop])}`);
  }
  return params.join("&");
}
 ...
 * @param {String} s
 */
function fromParams(s) {
  if (s && s.length > 0) {
    let params = s.split("&");
    let o = {};
    for (let i = 0; i < params.length; i++) {
      let param = params[i].split("=");
      o[param[0]] = decodeURI(param[1]);
    }
    return o;
  } else return {};
}
