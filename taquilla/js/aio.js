...
var init = function () {
    ...
    function cesto_realizarVenta(meta) {
      ...
      for (var i = 0; i < cesto.length; i++) {
        ...
        if (cesto[i].monto < $meta.vnt_min_num) {
          const sorteo = $sorteos.find((s) => s.sorteoID == cesto[i].sorteoID);
          const elementos = $elementos[sorteo.sorteo];
      ...
      return new Promise((resolve, reject) => {
        ...
        if (!numeros) {
          recargarElementosSync().then(() => {
            elementoSorteo(sorteo, numero).then((id) => resolve(id));
          });
        } else {
  ...
  function reporteOperadoras_nav(p, args) {
    ...
    function buscar(e) {
      e.preventDefault(e);
      const data = formControls(form);

      sqlAPISocket("2e1dd9aed9c21e58f649de33fc56c006", data, socket).then(
        (result) => {
          reporte.html(jsrender(rdReporte, result));
        }
      );
    }
