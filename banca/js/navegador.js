...
function bancasTaquillas_nav(p, args) {
  ...
  function updateView() {
    ...
    toggles.on("toggle", function (e, act) {
      ...
      socket.sendMessage("taquilla-activa", {
...
function bancasTaquilla_nav(p, args) {
  ...
  if (args && args.length == 1) {
    ...
    $('#meta').submit(function (e) {
      ...
      socket.sendMessage("taquilla-metas", {
...
function reporteGeneral_nav(p, args) {
  ...
  function init() {
    reporte.submit(function (e) {
      e.preventDefault(e);
      var data = formControls(this);
      buscarReporte(data, grp.val())
      nav.setURL(`#reporte/general|${[f1.val(),f2.val(),grp.val()].join('|')}`)
    });
  }
  init();

  function buscarReporte(data, grupo) {
    formLock(reporte);
  ...
  if (args && args.length > 0) {
    ...
    if (args[2]) {
      setTimeout(() => grp.select2('val', args[2]), 100);
      buscarReporte(busq, args[2])
    } else buscarReporte(busq, 0)
...
function reporteSorteo_nav(p, args) {
  ...
  var ecomisionTaq = $('#mnt-comision-taq');
  ...
  function reporte_mostrar(d) {
    ...
    d.forEach(item => {
      venta += item.jugada
      premios += item.premio
      item.comisionMonto = parseFloat(item.jugada * item.comision / 100)
      comisionAgencia += item.comisionMonto
    });

    $('#reporte-body').html(jsrender($('#rd-reporte'), d));
    eventa.html(venta.format())
    epremios.html(premios.format())
    ebalance.html((venta - premios - comisionAgencia).format())
    ecomisionTaq.html(comisionAgencia.format())
  }
...
function smsNuevo_nav(p, args) {
  select2w($('.s2dest'), {
    placeholder: "Dejar en blanco para enviar mensaje a su banquero/administrador"
  });
  ...
  socket.sendMessage("taquillas-act", {
    activa: 1
  }, function (e, d) {
    d = d || [];
    d.push();
