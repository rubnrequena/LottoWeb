...
function sorteoPublicar_nav(p, args) {
  ...
  var hlp = {
    taq: function (id) {
      if (id > 0) {
        ...
        return tqq
          ? tqq.nombre
          : "<span class='label label-danger'>TAQUILLA NO EXISTE</span>";
      } else return "TODAS";
    },
    srt: function (id) {
  ...
  if ($taquillas) {
    $("#taquillas").html(
      jsrender($("#rd-taquilla-option"), $taquillas.filter(soloActivas))
    );
    listSorteos();
  } else {
...
function bancasTopes_nav(p, args) {
  ...
  function dsp_topes(e, d) {
    ...
    $("#topes-body").html(jsrenderMerge($("#rd-topes-row"), d || [], help));
...
function reporteGeneral_nav(p, args) {
  ...
  function init() {
    reporte.submit(function (e) {
      ...
      nav.setURL(
        `#reporte/general|${[f1.val(), f2.val(), grp.val()].join("|")}`
      );
    });
  }
  init();
...
function reporteTaquilla_nav(p, args) {
  ...
  if ($taquillas) {
    ...
    socket.sendMessage("taquillas", null, function (e, d) {
      $taquillas = d;
      $("#taquillas").html(
        jsrender($("#rd-taquilla-option"), d.filter(soloActivas))
      );
      hbtaq.remove();
    });
  }
...
function reporteDiario_nav(p, args) {
  ...
  function updateView() {
    ...
    rpt.forEach(function (item) {
      item.balance =
        item.jugado - (premios.val() == 0 ? item.pago : item.premio);
...
function sms_nav(p, args) {
  socket.sendMessage("sms-bandeja", null, function (e, d) {
    ...
    $(".star").click(function () {
      ...
      } else jQuery(this).removeClass("star-checked");
      return false;
    });
...
function smsNuevo_nav(p, args) {
  select2w($(".s2dest"), {
    placeholder:
      "Dejar en blanco para enviar mensaje a su banquero/administrador",
  });
  $("#wys").html(
